...
const librerias = ["Recientes","Artistas","Álbumes","Canciones","Estaciones"]
    ...
    <div className="w-80	bg-red-900	h-11/12 space-y-7 xl:block hidden">
      ...
      <div className="flex flex-col space-y-1">
        ...
        { librerias.map((libreria)=>{
          return <p key={libreria} className="text-white border-l-4 cursor-pointer	border-red-900 hover:border-red-400 hover:text-red-400  pl-9">{libreria}</p>
        })}
      </div>
      <div className="flex flex-col space-y-1">
        <p className="text-white pl-9 text-xl border-l-4 border-red-900 ">Playlist</p>
        { playlists.map((playlist)=>{
          return <p key={playlist} className="text-white border-l-4 cursor-pointer	border-red-900 hover:border-red-400 hover:text-red-400  pl-9">{playlist}</p>
        })}
      </div>
    </div>
  )
